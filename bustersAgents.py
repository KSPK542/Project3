...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            Temporary_position = dist.argMax()
            Temporary_distance = self.distancer.getDistance(pacmanPosition, Temporary_position)
            if Temporary_distance < distance:
                distance = Temporary_distance
                G_Pos = Temporary_position

        dist = float("inf")
        action = None
        for a in legal:
            next_pos = Actions.getSuccessor(pacmanPosition, a)
            temp = self.distancer.getDistance(next_pos, G_Pos)
            if temp < dist:
                dist = temp
                action = a
        return action
